...
class Student:
    ...
    with open('C:/Users/ACER/Downloads/Try PaSCAN/Try PaSCAN/resource/student.json') as f:
...
class LogExport:
    ...
    def export(self, attendance):
        ...
        worksheet.write_string(0, 1, 'Left from School Premises', bold)  # This will write an empty string to the position "0,1"
...
class Export:
    ...
    def export(self, attendance):
        ...
        # Remove the position for "0,1" in the Excel file
        worksheet = writer.sheets[str(key)]
        worksheet.write_string(0, 1, 'Arrival To School Premises', bold)  # This will write an empty string to the position "0,1"
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def closeEvent(self, event):
        ...
        config = ConfigParser()
        config.read(r'C:/Users/ACER/Downloads/Try PaSCAN/Try PaSCAN/config/config.ini')  # Provide the path to your config.ini if it's not in the current directory
        ...
        file_path_for_Data = r"C:/Users/ACER/Downloads/Try PaSCAN/Try PaSCAN/database/attendance/"
        file_for_output = r"C:/Users/ACER/Downloads/Try PaSCAN/Try PaSCAN/database/attendance/Combined.xlsx"
        ...
        try:
            ...
            empty_df = pd.DataFrame([[''] * df1.shape[1]], columns=df1.columns)

            # Combine the data vertically with an empty row in between
            combined_df = pd.concat([df1, empty_df, df2], ignore_index=True)

            # Write the combined data to a new Excel file
            combined_df.to_excel(file_for_output, index=False, header=False)

            # Load the workbook and select the active sheet
            wb = load_workbook(file_for_output)
            ws = wb.active
